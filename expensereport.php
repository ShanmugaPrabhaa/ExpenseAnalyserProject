...
// Predict expenses for each category based on the previous month's data
$prev_month = ($current_month - 1 < 1) ? 12 : $current_month - 1;
$prev_year = ($current_month - 1 < 1) ? $current_year - 1 : $current_year;
...
foreach ($categories as $category) {
    $query = "SELECT DAY(date) as day, SUM(amount) as daily_total 
              FROM Expense e
              JOIN Categories c ON e.category_id = c.category_id
              WHERE e.user_id = ? AND c.category_name = ? AND YEAR(date) = ? AND MONTH(date) = ?
              GROUP BY DAY(date)";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('isii', $user_id, $category, $prev_year, $prev_month);
    $stmt->execute();
    $result = $stmt->get_result();

    $prev_month_totals = array_fill(0, cal_days_in_month(CAL_GREGORIAN, $prev_month, $prev_year), 0);
    while ($row = $result->fetch_assoc()) {
        $prev_month_totals[$row['day'] - 1] = $row['daily_total'];
    }
    $stmt->close();

    $n = count($prev_month_totals);
    $sum_x = array_sum(range(1, $n));
    $sum_y = array_sum($prev_month_totals);
    $sum_xy = 0;
    $sum_xx = 0;
    for ($i = 0; $i < $n; $i++) {
        $sum_xy += (($i + 1) * $prev_month_totals[$i]);
        $sum_xx += (($i + 1) * ($i + 1));
    }
    ...
    for ($day = 1; $day <= cal_days_in_month(CAL_GREGORIAN, $next_month, $next_year); $day++) {
        $predicted_expense = $slope * $day + $intercept;
        $predicted_category_expenses[] = max($predicted_expense, 0); // Ensure no negative predictions
    }

    $category_predictions[$category] = array_sum($predicted_category_expenses);
}
    ...
    <main id="main" class="main">
        ...
        <div class="row">
           ...
           <!-- <div class="col-lg-6">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">Predicted Expenses by Category for Next Month</h5>
                        <?php foreach ($category_predictions as $category => $predicted_amount): ?>
                            <p><strong><?php echo htmlspecialchars($category); ?>:</strong> <?php echo number_format($predicted_amount, 2); ?></p>
                        <?php endforeach; ?>
                    </div>
                </div>
            </div> -->
